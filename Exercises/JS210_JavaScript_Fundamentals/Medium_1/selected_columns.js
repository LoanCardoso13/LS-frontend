...
function getSelectedColumns(numbers, cols) {
  var result = [];
  let lengthRow = numbers.length;

  for (let rowIdx = 0; rowIdx < lengthRow; rowIdx += 1) {
    for (let columnIdx = 0, lengthColumn = cols.length; columnIdx < lengthColumn; columnIdx += 1) {
      if (!result[columnIdx]) {
        result[columnIdx] = [];
      }

      result[columnIdx][rowIdx] = numbers[rowIdx][cols[columnIdx]];
    }
  }
